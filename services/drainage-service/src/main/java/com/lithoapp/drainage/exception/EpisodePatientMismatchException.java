 ...
 *
 * Domain rule: every drainage record must belong to a specific episode, and that
 * episode must belong to the patient supplied in the request.
 * Accepting a mismatched pair would silently corrupt the patient → episode → drainage chain.
 *
...
public class EpisodePatientMismatchException extends RuntimeException {
    ...
    public EpisodePatientMismatchException(Long episodeId, Long patientId) {
        super(String.format(
                ...
                "A drainage record must be linked to an episode that belongs to the same patient.",
                episodeId, patientId));
    }
}
