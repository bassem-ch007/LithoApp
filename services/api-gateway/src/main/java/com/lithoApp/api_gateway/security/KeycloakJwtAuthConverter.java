...
/**
 * Gateway-side JWT converter — extracts Keycloak realm roles so that
 * Spring Security can see ROLE_ADMIN / ROLE_UROLOGUE / ROLE_BIOLOGIST.
 *
 * The gateway only authenticates requests; individual microservices
 * enforce RBAC locally. This converter keeps the gateway's security
 * context consistent with downstream services.
 */
@Component
public class KeycloakJwtAuthConverter implements Converter<Jwt, Mono<AbstractAuthenticationToken>> {
    ...
    public Mono<AbstractAuthenticationToken> convert(Jwt jwt) {
        ...
        return Mono.just(new JwtAuthenticationToken(jwt, combined,
